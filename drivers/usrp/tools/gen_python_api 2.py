...
def parse_help(text):
    ...
    for raw in text.splitlines():
        ...
        # group 2 matches EITHER " arg" (plain value option) OR " [=arg(=IMPL)]"
        # (a value<bool> with implicit_value, boost's config-file-friendly flag);
        # group 3 is the "(=default)"; group 4 the description.
        m = re.match(
            r'^  (--[A-Za-z0-9_-]+)(\s+arg|\s+\[=arg\(=[^)]*\)\])?(?:\s+\(=(.*?)\))?(?:\s+(.*\S))?\s*$',
            line)
        if m:
            if cur:
                opts.append(cur)
...
def emit_md(opts):
    ...
    for name, ha, h, d in opts:
        ...
        defmap[name] = d
    L = []
    L.append("# All controllable parameters")
    L.append("")
