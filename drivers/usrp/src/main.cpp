...
int UHD_SAFE_MAIN(int argc, char* argv[])
{
    ...
    // (keeps each description clean for the auto-generated Python API / PARAMETERS.md).
    po::options_description desc("Allowed options", 200);
    desc.add_options()
        ("help", "show this help message")
        ("config", po::value<std::string>(),
                     "read options from a config file (one 'name = value' per line, "
                     "'#' comments; use the long option name without '--'). Any option "
                     "given on the COMMAND LINE overrides the file. Generate a fully "
                     "defaulted template with:  sdr_system --help  (or see phy.cfg).")
        ...
        ("marl-report", po::value<bool>(&marl_report)->default_value(false),
                     "role rx: emit one machine-readable line per CRC-OK burst — "
                     "'[BURST] id=<payload byte0> idx=<i> tot=<t> nbytes=<n> hex=<HEX>' — "
                     "for the MARL multi-agent AP to route an ACK to the transmitting "
                     "agent (payload byte 0 = agent id). No effect on the decoded message.")
        ...
        ("serve-forever", po::value<bool>(&serve_forever)->default_value(false)->implicit_value(true),
        ...
        ("on-demand", po::value<bool>(&on_demand)->default_value(false)->implicit_value(true),
        ...
        ("chirp-down", po::value<bool>(&chirp_down)->default_value(false)->implicit_value(true),
        ...
        ("fec_soft",
         po::value<bool>(&config.fec_soft)->default_value(false)->implicit_value(true),
        ...
        ("fec-type",
         po::value<std::string>(&config.fec_type)->default_value("conv"),
         "FEC code family (needs --fec true; must MATCH on both ends): "
         "conv = rate-1/2 K=7 convolutional + Viterbi (default), "
         "ldpc = rate-1/2 systematic IRA/staircase LDPC + min-sum belief propagation, "
        ...
        ("ldpc-k",
         po::value<int>(&config.ldpc_k)->default_value(256),
        ...
        ("fec-iters",
         po::value<int>(&config.fec_iters)->default_value(0),
         "Tuning: max decoder iterations (LDPC belief-prop / turbo BCJR). 0 = default "
         "(LDPC 50, turbo 6). Raise (e.g. turbo 8-12) if a marginal link won't "
         "converge. Decoder-only — need not match the other end.")
        ("fec-scale",
         po::value<float>(&config.fec_scale)->default_value(0.0f),
        ...
        ("ldpc-col-weight",
         po::value<int>(&config.ldpc_col_weight)->default_value(3),
    ...
    po::variables_map vm;
    // Command line first — boost keeps the FIRST stored value for each option, so
    // anything on the CLI overrides the config file (you change only what you need).
    po::store(po::parse_command_line(argc, argv, desc), vm);
    if (vm.count("config")) {
        std::string cfg = vm["config"].as<std::string>();
        std::ifstream cfg_in(cfg);
        if (!cfg_in) {
            std::cerr << "[MAIN] --config: cannot open '" << cfg << "'\n";
            return EXIT_FAILURE;
        }
        try {
            po::store(po::parse_config_file(cfg_in, desc), vm);   // fills in the rest
            std::cout << "[MAIN] loaded config file: " << cfg << "\n";
        } catch (const std::exception& e) {
            std::cerr << "[MAIN] --config parse error in '" << cfg << "': " << e.what() << "\n";
            return EXIT_FAILURE;
        }
    }
    po::notify(vm);
    ...
    // ── Select the FEC code family ──────────────────────────
    // Must run before any fec_encoded_len()/encode/decode below so both the
    // symbol-sizing math and the runtime codec agree. No-op unless --fec is on,
    // but selecting it unconditionally keeps the encoded-length helper correct.
    fec_set_tuning(config.fec_iters, config.fec_scale, config.ldpc_col_weight);
    fec_set_type(config.fec_type, config.ldpc_k);
    if (config.fec) {
        const char* fam = fec_is_ldpc() ? "LDPC" : (fec_is_turbo() ? "turbo" : "conv (K=7 Viterbi)");
        std::cout << "[MAIN] FEC: " << fam
                  << (config.fec_soft ? " soft-decision" : " hard-decision")
                  << ((fec_is_ldpc() || fec_is_turbo()) ? " (k=" + std::to_string(config.ldpc_k) + ")" : "")
                  << "\n";
    }
    ...
    if (config.waveform == "lora" && (config.role == "tx" || config.role == "rx")) {
        ...
        if (config.role == "tx") {
            ...
            while (!global_stop_signal.load()) {
                ...
                if (config.fec) bits = fec_decode_block(bits, pre_bits);
    ...
    if (config.role == "source_arq" && on_demand) {
    ...
    } else if (config.role == "tx" && on_demand) {
        // WARM ONE-WAY TRANSMITTER: the radio (started above) stays warm; transmit
        // the framed packet ONCE per stdin line, then print "SENT". Unlike the finite
        // role tx (which re-opens the radio each run), the LO stays settled between
        // fires -> reliable single-shot decode. Pairs with the MARL AP (ap_multi),
        // which ACKs by decoding the id-tagged burst; no ARQ/ACK here (one-way).
        std::cout << "[TX] on-demand warm transmitter: one burst per stdin line ('go'), "
                     "EOF to stop. Radio stays warm.\n";
        std::cout.flush();
        ...
        while (std::getline(std::cin, line) && !global_stop_signal.load()) {
            ...
            // Settle briefly so the pipeline picks up the burst, then wait for it to
            // drain onto the air before reporting — so the caller fires the next
            // packet only after this one is transmitted (bounded so it can't wedge).
            std::this_thread::sleep_for(std::chrono::milliseconds(50));
            for (int w = 0; w < 200 && transceiver.tx_pending() > 0
                                    && !global_stop_signal.load(); ++w)
        ...
        while (!global_stop_signal.load()) {
            ...
            if (transceiver.rx_bits_fifo.pop(rx)) {
                ...
                if (config.fec) {
                    int info_len = 16 + (int)bytes_length * 8 + 16;
                ...
                crc_pass++;
                // MARL multi-agent AP: emit a machine-readable line per CRC-OK
                // burst so the Python AP can route an ACK to the transmitting agent
                // (payload byte 0 = agent id). Printed for EVERY good burst
                // (including duplicates), regardless of message-type, and flushed so
                // the AP reads it immediately. hex carries the full payload in case
                // the AP needs more than the id.
                if (marl_report) {
                    static const char* HX = "0123456789ABCDEF";
                    std::string hex;
                    for (unsigned char c : payload) { hex += HX[c >> 4]; hex += HX[c & 0xF]; }
                    int id = payload.empty() ? -1
                                             : (int)(unsigned char)payload[0];
                    std::cout << "[BURST] id=" << id
                              << " idx=" << (int)idx << " tot=" << (int)tot
                              << " nbytes=" << payload.size()
                              << " hex=" << hex << std::endl;   // std::endl flushes
                }
                total = tot;
