...
- **`experiments/`** — your code (one folder per algorithm); **`run.sh`** runs it.
- **`drivers/usrp/build/sdr_system`** — the C++ radio engine (the modem); **`radio.sh`** runs raw TX/RX.
- **`drivers/usrp/python/`** — the Python you call: `run_algo.py`/`phy_link.py` (uniform API),
  `sdr.py`/`run.py`/`configs/` (direct radio), `channel_sense.py`/`freq_scan.py` (RF utils).
- **`drivers/usrp/bindings/pyphy`** — the DSP blocks as numpy functions.
- **`experiments/`** — worked apps (MARL, FL, CLIP, jammer).

**Full repo layout → [`STRUCTURE.md`](../../docs/STRUCTURE.md); file-by-file index → [`MANIFEST.md`](../../docs/MANIFEST.md).**
...
./run.sh list                     # list the algorithms in experiments/
...
`experiments/<name>/app.py` (copy `experiments/_template/`), then run it by name. **Step-by-step
...
```python
# experiments/<name>/app.py — no framework import, no radio code
import numpy as np
class MyAlgo:
...
See `experiments/README.md` for the full contract and the worked examples (`fl`, `clip_semcom`,
`marl`, `plain_echo`).
...
- `load_app_factory(name)` — find/adapt the algorithm in `experiments/<name>/app.py`
...
- **`experiments/EXPERIMENT_GUIDE.pdf`** — step-by-step commands to run the example apps.
- **`experiments/README.md`** — how to write/upload an algorithm.
