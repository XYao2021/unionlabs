...
reproduces the *physics and the reward* of the original paper (MARL_RA_Union), rather
than the toy ideal-collision model. N agents (TX) contend for one access point (RX).

Channel  (paper's `wireless_channel`, MARL_network_Union.py + MARL_setting_Union.py):
...
Reward  (paper objective 2 = "fair rate", MARL_RA_Union.py:282-285):
...
Reuses the REAL actor_MLP/critic_MLP from applications/MARL_RA_Union. Needs torch.
...
from MARL_learning_Union import actor_MLP, critic_MLP            # the REAL nets  # noqa: E402

# ── paper PHY constants (MARL_setting_Union.py) ──
