...
| `marl_ra/marl_ra.py` … `MARL_base/network/learning/setting_Union.py` | The RL code: environment, base classes, actor/critic networks, training, settings. |
| `marl_ra/marl_phy.py` · `real_channel.py` | PHY bridge (warm source/AP, BER probe) / the reusable real-radio channel. |
| `marl_ra/ap_multi.py` · `agent_node.py` · `slot_sync.py` · `mock_medium.py` | Multi-agent AP + ACK routing / decentralized agent / slot clock / offline medium. |
| `marl_ra/marl_env.py` · `marl_train.py` · `marl_multi_env.py` · `marl_multi_train.py` · `aloha_baseline.py` | Single-agent env+trainer / multi-agent env+trainer / q-ALOHA baseline. |
| `marl_ra/README.md` · `INTEGRATION.md` · `requirements.txt` · `run_training.sh` | Overview / design note / deps / training launcher. |
| `fl/fl.py` · `fl_core.py` · `mnist_sgd_over_sdr.py` | FedAvg app / shared FL library / MNIST SGD-over-SDR base. |
| `clip_semcom/semcom.py` · `semcom_core.py` · `phy_port.py` | CLI app / CLIP backend+codec+classifier / the PHY API port. |
| `clip_semcom/README.md` · `INTEGRATION.md` · `requirements.txt` | Overview / integration design note / optional deps. |
| `stc_aircomp/README.md` · `INTEGRATION.md` | Design + build plan for STLC over-the-air computation (not yet built). |
...
| File | Description |
|---|---|
| `initialization.sh` | Installs the toolchain (`--build` also compiles `drivers/usrp/build/sdr_system`). |
