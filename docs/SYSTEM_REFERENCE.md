...
(`drivers/usrp/tools/gen_config_template.py`) from `sdr_system --help`, so it never drifts from the code.
...
`drivers/usrp/src/main.cpp` (`--waveform lora`). Unlike the constellation pipeline (m-sequence
