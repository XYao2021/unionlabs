...
| `--bytes-length` | value | `125` | payload bytes per chunk (default 125). Larger chunks amortise the per-burst detect/sync/ACK overhead — higher throughput. MUST match on TX and RX. The receiver accepts at most 64 chunks per message (a sanity guard on the header, not a field limit — idx and tot are 8-bit), so the largest message is 64 x this value. |
