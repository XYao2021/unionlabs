...
How the `marl_ra` multi-agent RL random-access code hooks onto the USRP B210
...
the actor networks in `marl_learning.py`.
...
3. **Online training on the real link — DONE.** `marl_train.py`: single-agent A2C reusing
   `marl_learning.Actor_A2C` + `Critic` over `RealChannelEnv`. Per step: obs → actor
