...
uniform API as the **compute archetype** (`experiments/stc_aircomp/`, `--role aircomp`):

```
python3 experiments/stc_aircomp/stc_aircomp.py --sensors 8 --bits 4   # NMSE-vs-SNR figure
...
| NMSE-vs-SNR evaluation + uniform-API port | **built** — `stc_aircomp.py`, `experiments/stc_aircomp/` |
