...
Reads viz/{tx_symbols,tx_wave,rx_symbols,rx_wave}.txt (each "real imag" per line)
...
def main():
    ...
    fig.suptitle(f"SDR signals  (viz_dir={viz_dir}, fs={fs/1e6:.2f} MHz)", fontsize=13)
    ...
    for row, side in enumerate(["tx", "rx"]):
        ...
        a = ax[row][2]
        if syms is not None:
            a.scatter(np.real(syms), np.imag(syms), s=6, color=color, alpha=0.4)
            lim = 1.6 * (np.max(np.abs(syms)) if len(syms) else 1)
            a.set_xlim(-lim, lim); a.set_ylim(-lim, lim)
            a.axhline(0, color="k", lw=0.4); a.axvline(0, color="k", lw=0.4)
            a.set_aspect("equal")
        a.set_title(f"{side.upper()} constellation"); a.set_xlabel("I"); a.set_ylabel("Q")
        a.grid(True, alpha=0.3)
