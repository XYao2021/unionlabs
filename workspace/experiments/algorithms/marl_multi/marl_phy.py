...
SINGLE-SHOT semantics (see ../experiments/marl_ra/INTEGRATION.md §2): the
...
def _phy(a):
    ...
    d = dict(
        ...
        rx_rate=a.get("rx_rate", 1.6e6), tx_rate=a.get("tx_rate", 1.6e6),
        rx_subdev=a.get("rx_subdev", "A:A"), tx_subdev=a.get("tx_subdev", "A:A"),
        rx_ant=a.get("rx_ant", "RX2"), tx_ant=a.get("tx_ant", "TX/RX"),
    ...
    # symbol_rate: only override the C++ default (0.8e6) when explicitly set. A
    # non-default sample rate (e.g. 2e6 for an N210-exact link) REQUIRES a matching
    # symbol_rate so tx_rate == symbol_rate*U/D holds, else the startup rate-chain
    # consistency check fails. Defaults untouched, so existing callers are unchanged.
    if a.get("symbol_rate") is not None:
        d["symbol_rate"] = a["symbol_rate"]
