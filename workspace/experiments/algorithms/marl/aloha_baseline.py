...
marl_ra. This runs it over `marl_env.RealChannelEnv` (same device model,
...
(inference) on the same link and overlays it as a reference.
    ...
    python3 aloha_baseline.py --sweep 0.25,0.5,0.75,1.0 --steps 100 \
        --actor ../experiments/marl_ra/results/real_dqpsk_400.pt
...
def actor_policy(path, dim_O, dim_A):
    ...
    from marl_setting import LearningSettings
...
def main(argv):
    ...
    a.add_argument("--out", default=os.path.join(
        ...
        "..", "marl_multi", "results", "aloha_vs_marl.png"))
