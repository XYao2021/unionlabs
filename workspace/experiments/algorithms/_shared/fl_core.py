...
def label_shards(X, y, K, shards_per_node=2, seed=0):
    """NON-IID split (the FedAvg label-skew protocol): sort by label, cut into
    K*shards_per_node contiguous blocks, and deal shards_per_node blocks at random to each
    node — so a node sees only a couple of digits, not all ten. This is where the topology
    and the aggregation rule start to matter: with IID shards every node learns nearly the
    same model anyway."""
    rng = np.random.RandomState(seed)
    order = np.argsort(y, kind="stable")                 # group the samples by label
    blocks = np.array_split(order, K * shards_per_node)
    pick = rng.permutation(len(blocks))
    out = []
    for k in range(K):
        idx = np.concatenate([blocks[b] for b in pick[k * shards_per_node:(k + 1) * shards_per_node]])
        idx = rng.permutation(idx)                       # shuffle within the node's own data
