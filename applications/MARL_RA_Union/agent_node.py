...
class MockMediumClient:
    ...
    def wait_slot(self):
        # mock_medium.py already barrier-syncs each step(), so slotting is a no-op here.
        return
...
class RealMediumClient:
    ...
    def __init__(self, agent_id, tx_args, sense_rx_args=None, ap_host="127.0.0.1",
                 ap_port=5599, tx_gain=89, scheme="QPSK", threshold_db=None,
                 slot_host=None, slot_port=5600, slot_ms=150, **opts):
        from marl_phy import SenseStream                      # noqa
        from ap_multi import AgentAckClient
        self.id = agent_id
        # Slotted mode: announce a 'TX' intent to the AP each fire (so it arbitrates
        # the slot -> logical collision), and wait ~1.2 slots for the verdict (the AP
        # arbitrates at the next tick). So the collision-detection wait is slot-scaled,
        # and a collision naturally costs the agent ~1 slot of backoff.
        slotted = slot_host is not None
        ack_to = max(1.0, slot_ms / 1000.0 * 1.2) if slotted else 3.0
        # Transport that MATCHES ap_multi: a persistent HELLO socket + one-way
        # id-tagged burst + wait-for-ACK. AgentAckClient tags byte 0 = agent id.
        self._tx = AgentAckClient(agent_id, tx_args=tx_args, ap_host=ap_host,
                                  ap_port=ap_port, tx_gain=tx_gain, scheme=scheme,
                                  ack_timeout=ack_to, announce_intent=slotted, **opts)
        self._sense = SenseStream(rx_args=sense_rx_args) if sense_rx_args else None
        self._thr = threshold_db
        # Slot clock: every step waits for the shared 'SLOT k' tick so all agents act
        # in aligned slots (real contention). Omit slot_host -> async (previous behavior).
        self._slot = None
        if slotted:
            from slot_sync import SlotClient
            self._slot = SlotClient(slot_host, slot_port)

    def wait_slot(self):
        if self._slot is not None:
            self._slot.wait_slot()
...
def run_agent(client, agent_id, slots, objective=0, pkt_int=3, Q_max=50, lr=4e-4,
    ...
    for t in range(slots):
        # Slotted operation: block until the shared 'SLOT k' tick so every agent
        # decides+transmits in the SAME slot (real contention). No-op when no slot
        # clock is configured (async — previous behavior).
        if hasattr(client, "wait_slot"):
            client.wait_slot()
...
def main(argv):
    ...
    a.add_argument("--tx-subdev", default="A:A", help="B210=A:A")
    a.add_argument("--tx-ant", default="TX/RX")
    a.add_argument("--sample-rate", type=float, default=None,
                   help="tx_rate == rx_rate; set to an N210-exact rate (e.g. 2e6) when the AP is an N210")
    a.add_argument("--symbol-rate", type=float, default=None,
                   help="set with --sample-rate so rate==symbol_rate*U/D (e.g. 1e6 for 2e6)")
    a.add_argument("--bytes-length", type=int, default=125,
                   help="payload bytes per packet — MUST match the AP (default 125)")
    a.add_argument("--slot-host", default=None,
                   help="host running slot_sync.py; set to run SLOTTED (all agents aligned). Omit = async")
    a.add_argument("--slot-port", type=int, default=5600)
    a.add_argument("--slot-ms", type=int, default=150,
                   help="slot duration in ms — MUST match slot_sync.py --slot-ms (sizes the ACK wait)")
    ...
    if args.mock:
        ...
        # PHY overrides threaded to the agent's WarmSource (-> marl_phy._phy). Defaults
        # match today's behavior; set --sample-rate/--symbol-rate for an N210 AP link.
        phy_opts = dict(tx_subdev=args.tx_subdev, tx_ant=args.tx_ant,
                        packet_bytes=args.bytes_length)
        if args.sample_rate is not None:
            phy_opts["tx_rate"] = args.sample_rate
            phy_opts["rx_rate"] = args.sample_rate      # source_arq checks rx_rate==tx_rate
        if args.symbol_rate is not None:
            phy_opts["symbol_rate"] = args.symbol_rate
        client = RealMediumClient(args.id, tx_args=args.tx_args,
                                  sense_rx_args=args.sense_rx_args, ap_host=args.ap_host,
                                  ap_port=args.ap_port, tx_gain=args.tx_gain,
                                  scheme=args.scheme, slot_host=args.slot_host,
                                  slot_port=args.slot_port, slot_ms=args.slot_ms, **phy_opts)
