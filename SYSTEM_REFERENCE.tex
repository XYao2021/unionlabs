...
\tightlist
\item
  Binary: \texttt{phy/build/sdr\_system}
\item
...
\includegraphics{results/figures/constellations.png}

\includegraphics{results/figures/ber_curves.png}
...
\includegraphics{results/figures/rrc.png}
...
\includegraphics{results/figures/ofdm_orthogonality.png}
...
\includegraphics{results/figures/energy_detector.png}
...
\hypertarget{41-channel-sensing-occupancy--listen-before-talk}{%
\subsubsection{4.1 Channel sensing (occupancy /
listen-before-talk)}\label{41-channel-sensing-occupancy--listen-before-talk}}

The same energy measurement, used for a different decision: not
\emph{"is a burst starting now"} but \emph{"is the channel occupied over
this window"} --- the basis for carrier-sense multiple access (CSMA).
\texttt{-\/-role\ sense} streams samples (no decode pipeline),
integrates the average power over a window of \texttt{-\/-sense-window}
ms, and declares the band \textbf{busy} when it exceeds a threshold:
...
\texttt{{[}SENSE{]}\ busy=..\ power\_db=..\ peak\_db=..\ samples=..} ---
for \texttt{-\/-sense-count} windows (\textbf{\texttt{0} = stream
forever}, a persistent feed). It is the same \(H_0/H_1\) power test as
§4, but integrated over a \emph{fixed window} the caller chooses rather
than gated to a burst envelope.

\textbf{Calibration matters.} The absolute power depends on
\texttt{-\/-rx-gain} and the ambient/DC-leakage floor, so
\(\gamma_\text{dB}\) must be set \emph{relative to the measured idle
floor}, not hard-coded. The Python layer (\texttt{channel\_sense.py})
auto-calibrates: it measures the idle floor over N windows and sets
\(\gamma_\text{dB} = \operatorname{median}(P_\text{idle,dB}) + \text{margin}\)
(default +6 dB). Validated on this rig: idle floor ≈ \textbf{−12 dB}, an
active carrier ≈ \textbf{−3 dB} (a clean +9 dB separation), threshold
auto-placed between them.

\textbf{Reusable API} (\texttt{channel\_sense.py}, importable from any
script):

\begin{itemize}
\tightlist
\item
  \texttt{sense\_channel(...)} --- one measurement -\textgreater{}
  \texttt{\{busy,\ power\_db,\ \ldots{}\}} (re-inits the radio).
\item
  \texttt{calibrate\_floor(...)} --- idle floor -\textgreater{} busy
  threshold.
\item
  \texttt{SenseStream} --- a \textbf{persistent} feed
  (\texttt{-\/-sense-count\ 0}) with one radio init and a background
  reader that keeps the latest window fresh;
  \texttt{.should\_transmit(p,\ thr)} is an instant decision with no
  per-call re-init (Python owns the threshold; the feed streams raw
  \texttt{power\_db}).
\item
  \texttt{should\_transmit(p,\ \ldots{})} --- \textbf{p-persistent
  access}: if the channel is idle, transmit with probability \(p\); if
  busy, defer. The scaffold for CSMA/collision-avoidance on top of the
  PHY.
\end{itemize}

\textbf{Frequency-range scan} (\texttt{freq\_scan.py}) sweeps a band to
find a \emph{quiet carrier} before a run: it retunes the RX across
\texttt{-\/-start}/\texttt{-\/-stop} in \texttt{-\/-step} MHz, senses
the received power at each step (via \texttt{-\/-role\ sense}), and
prints a per-frequency table + an ASCII spectrum, ranking the quietest
candidates (with an optional power-vs-frequency plot). E.g.
\texttt{python3\ freq\_scan.py\ -\/-start\ 902\ -\/-stop\ 928\ -\/-step\ 1\ -\/-rx-args\ addr=192.168.20.2}.

\textbf{Common alternatives:} full CSMA/CA with randomized exponential
backoff, RTS/CTS handshaking, energy detection with hysteresis (separate
busy/idle thresholds to avoid chattering), and --- for sensing
\emph{below} the noise floor --- matched-filter or cyclostationary
detection as in §4.
...
\includegraphics{results/figures/gardner_scurve.png}
...
rad/sample. Three estimators are provided (\texttt{CFOEstimator}), all
\textbf{data-aided} off the preamble, then \texttt{FrequencyShifter}
removes it by multiplying sample \(n\) by \(e^{-j2\pi(\Delta f/f_s)n}\)
(a running phase accumulator, so blocks join seamlessly). The pipeline
default is \textbf{(C) least-squares phase-slope}.

\includegraphics{results/figures/cfo_effect.png}
...
estimator OFDM uses (§5.6), where \(L = N/2\). Note (B) needs a
\textbf{repeated} preamble; the single-carrier default preamble is a
single m-sequence, so (B) is not used there.

\textbf{(C) Least-squares phase-slope (default).} Strip the modulation
from every preamble symbol,
\(\theta_k = \arg(r[k\,\text{sps}]\,p^{*}[k]) = \varphi_0 + 2\pi(\Delta f/f_s)\,\text{sps}\,k + \text{noise}\),
unwrap, and fit a magnitude-weighted straight line; the slope \(b\)
gives \(\widehat{\Delta f} = b\,f_s/(2\pi\,\text{sps})\). Because it
uses \textbf{all} preamble symbols jointly (vs (A)'s lag-1 differencing)
this is the data-aided ML / CRLB estimate --- same
\(\pm f_s/(2\,\text{sps})\) range as (A) but
\textbf{\textasciitilde{}\(L^2\) lower variance}. A hardware-free
Monte-Carlo (length-31 m-seq) puts the CFO-estimate RMSE at
\textasciitilde450 Hz vs \textasciitilde1050 Hz for (A) at 12 dB symbol
SNR --- i.e. it roughly \textbf{halves} the \(\pm1200\) Hz jitter of §13
--- and it wins at every SNR the link actually decodes at (both fail
near 0 dB, where phase unwrap breaks down). A \textbf{cross-burst EMA
prior} (\texttt{-\/-cfo\_prior\_alpha}, default \texttt{1.0} =
per-burst) can further smooth the estimate for a \emph{warm resident
LO}; keep it at \texttt{1.0} for a cold per-fire LO (two-host DQPSK),
whose CFO changes every burst.
...
\includegraphics{results/figures/pll_convergence.png}
...
\includegraphics{results/figures/schmidl_cox.png}
...
\textbf{Soft-decision} Viterbi (Euclidean/LLR branch metric instead of
Hamming) is now implemented via \texttt{-\/-fec\_soft} ---
\textasciitilde2 dB over the hard-decision decoder (see §7.4).

The FEC is now \textbf{pluggable} ---
\texttt{-\/-fec-type\ \{conv,\ ldpc,\ turbo\}} selects one of three
rate-1/2 codes. The convolutional code above is the default; the
sections below cover the two added near-Shannon codes and the shared
selector/tuning and soft-decision layer.

\hypertarget{71-code-family-selector-and-tuning}{%
\subsubsection{7.1 Code-family selector and
tuning}\label{71-code-family-selector-and-tuning}}

\texttt{-\/-fec-type} (needs \texttt{-\/-fec\ true}) picks the code; all
three are rate 1/2, so the rest of the pipeline (symbol sizing, ARQ,
CRC) is unchanged, and both ends must select the same code. The
convenience layer in \texttt{fec.hpp} dispatches
\texttt{fec\_encoded\_len\ /\ fec\_encode\_block\ /\ fec\_decode\_block\ /\ fec\_soft\_decode\_block}
to the chosen codec; \texttt{fec\_set\_type()} builds it once at startup
and \texttt{fec\_set\_tuning()} applies the knobs. The block codes
(LDPC, turbo) segment the packet into \(k\)-bit blocks
(\texttt{-\/-ldpc-k}, default 256; last block zero-padded) and encode
each to \(2k\) coded bits.
...
\tightlist
\item
  \texttt{-\/-fec-iters\ N} --- max decoder iterations (LDPC
  belief-propagation / turbo BCJR; \texttt{0} = default 50 / 6). Raise
  (e.g. turbo 8--12) if a marginal link won't converge. Decoder-only ---
  need not match.
\item
  \texttt{-\/-fec-scale\ F} --- normalized min-sum (LDPC) / extrinsic
  (turbo) scale, 0.7--0.9 typical (\texttt{0} = default 0.75).
  Decoder-only.
\item
  \texttt{-\/-ldpc-k\ K} --- block size (LDPC \textbf{and} turbo).
  Larger = stronger, more latency/padding. \textbf{Must match both
  ends.}
\item
  \texttt{-\/-ldpc-col-weight\ W} --- LDPC variable-node degree (default
  3); higher = denser code. \textbf{Changes the parity-check matrix ---
  must match both ends} (turbo ignores it).
\end{itemize}

\hypertarget{72-ldpc-ldpchpp}{%
\subsubsection{\texorpdfstring{7.2 LDPC
(\texttt{ldpc.hpp})}{7.2 LDPC (ldpc.hpp)}}\label{72-ldpc-ldpchpp}}

Rate-1/2 systematic \textbf{IRA / staircase} LDPC. The parity-check
matrix \(H = [\,H_u \mid H_p\,]\) is built deterministically from a
fixed seed (identical on both ends): the information part \(H_u\) places
\texttt{col\_weight} ones per info column, balanced across the \(m=k\)
check rows; the parity part \(H_p\) is lower-bidiagonal (an
accumulator). That structure gives \(O(n)\) \textbf{systematic encoding}
--- the parity is a prefix-XOR of the info syndrome,
\(p_r = \bigoplus_{i \le r} s_i\) with \(s = H_u\,u\) --- and parity
variable nodes of degree 2 (a genuinely good short code, unlike the
degree-1 parity of an \(H=[P\mid I]\) construction).

\textbf{Decoding} is flooding \textbf{normalized min-sum} belief
propagation on the Tanner graph. Each iteration: every variable node
sums its channel LLR and the incoming check messages; every check node
returns \(\operatorname{sign} = \prod \operatorname{sgn}\) and
\(\operatorname{magnitude} =
\alpha \cdot \min |L|\) over its \emph{other} edges (attenuation
\(\alpha =\) \texttt{-\/-fec-scale}, default 0.75). It
\textbf{early-terminates} the moment the hard-decision syndrome
\(H\hat{x}=0\) clears, so decode cost falls as SNR rises. LLR convention
matches \texttt{soft\_demodulate\_llr} (positive = bit 0).

\hypertarget{73-turbo-turbohpp}{%
\subsubsection{\texorpdfstring{7.3 Turbo
(\texttt{turbo.hpp})}{7.3 Turbo (turbo.hpp)}}\label{73-turbo-turbohpp}}

Rate-1/2 \textbf{punctured parallel-concatenated convolutional code}
(PCCC): two identical \((7,5)\) recursive-systematic convolutional (RSC)
encoders --- one on the info bits, one on an interleaved copy
(deterministic interleaver) --- with the two parity streams punctured
(alternate bits) to reach rate 1/2. The RSC has feedback
\(g_0 = 1+D+D^2\) (\(7_8\)) and feedforward \(g_1 = 1+D^2\) (\(5_8\)),
memory 2 -\textgreater{} a \textbf{4-state} trellis.

\textbf{Decoding} is the classic iterative turbo decoder: two
soft-in/soft-out (SISO) \textbf{max-log-MAP (BCJR)} decoders exchange
\textbf{extrinsic} LLRs through the interleaver for a few iterations,
then take a hard decision on the combined LLR. Each SISO runs forward
(\(\alpha\)) and backward (\(\beta\)) recursions over the trellis with
branch metric
\(\gamma \propto \tfrac12\big(x_u(L_a+L_s) + x_p L_p\big)\), forms
\(L(u) = \max^{*}_{u=1}(\alpha+\gamma+\beta) - \max^{*}_{u=0}(\alpha+\gamma+\beta)\),
and passes the scaled extrinsic
\(L_e = \text{scale}\cdot(L - L_a - L_s)\) to the other decoder
(deinterleaved). Turbo has the \textbf{steepest waterfall} of the three
codes (best coding gain per dB), and like LDPC it early-terminates as
the link improves.

\hypertarget{74-soft-decision-decoding}{%
\subsubsection{7.4 Soft-decision
decoding}\label{74-soft-decision-decoding}}

\texttt{-\/-fec\_soft} (RX-side) feeds per-bit \textbf{LLRs} from
\texttt{soft\_demodulate\_llr()} (a max-log demapper on the equalized
symbols; positive = bit 0) to the decoder instead of hard bits:

\begin{itemize}
\tightlist
\item
  \textbf{Convolutional:} soft Viterbi (Euclidean/LLR branch metric) ---
  \textasciitilde2 dB over hard.
\item
  \textbf{LDPC / turbo:} soft is their \emph{native} input;
  hard-decision (mapping bits to \(\pm\)LLR) throws away most of their
  gain.
\end{itemize}

The single-carrier RX emits LLRs from the phase-corrected symbols.
\textbf{OFDM} now does too: \texttt{ofdm\_demodulation\_thread} computes
them from the per-subcarrier-equalized symbols (with a decision-directed
noise-variance estimate) and feeds \texttt{rx\_llr\_fifo}. Previously
the OFDM path produced only hard bits, so \texttt{-\/-fec\_soft} on OFDM
silently fell back to hard decision --- a large loss for the iterative
codes.

\textbf{Comparison (hardware-free, QPSK).} At 2 dB \(E_b/N_0\), decode
success is turbo-soft \(\approx\)90 \% \(>\) LDPC-soft \(\approx\)75 \%
\(>\) conv-soft \(\approx\)52 \%. Decode \emph{cost}: Viterbi is fixed
(it walks the full 64-state trellis every packet), whereas LDPC
(min-sum) and turbo (BCJR) early-terminate, so their cost drops as SNR
rises. Ranking at short block sizes: turbo = best gain (moderate cost),
LDPC = fastest with good gain, conv = weakest gain but predictable,
bounded latency.

\textbf{Common alternatives not implemented:} Reed-Solomon (burst
errors), Polar codes (5G control).
...
\hypertarget{81-measuring-the-real-ber---ber-expected}{%
\subsubsection{\texorpdfstring{8.1 Measuring the real BER
(\texttt{-\/-ber-expected})}{8.1 Measuring the real BER (-\/-ber-expected)}}\label{81-measuring-the-real-ber---ber-expected}}

CRC is a \textbf{binary} verdict --- a failed frame has \emph{≥1} error,
but CRC can't say whether that's one bit or hundreds. To quantify it,
give the sink the \textbf{known transmitted payload}
(\texttt{-\/-ber-expected\ \textless{}file\textgreater{}}) and it
compares the decoded bits to ground truth for \textbf{every} burst (CRC
pass or fail), printing two rates:

\begin{itemize}
\tightlist
\item
  \textbf{pre-FEC BER} --- raw channel bit errors (demodulated bits vs
  the transmitted \emph{coded} bits). This is the true channel quality,
  before any correction.
\item
  \textbf{post-FEC payload BER} --- residual errors after Viterbi
  decoding (decoded payload vs the known payload). This is what the CRC
  actually sees; non-zero here =\textgreater{} CRC fails.
\end{itemize}

\[ \text{BER} = \frac{\#\{\text{bits that flipped}\}}{\#\text{bits}}. \]

So a CRC-fail frame is \textbf{not necessarily garbage} --- in a
\emph{marginal} window you see e.g. \texttt{pre-FEC\ 1.2\%},
\texttt{post-FEC\ 0.3\%} (nearly right, CRC catching 1--2 residual
bits). The key diagnostic signature is \textbf{post-FEC \textgreater{}
pre-FEC}: once the raw BER exceeds the rate-½ K=7 code's decoding
threshold (\textasciitilde10--11\%), the Viterbi decoder chooses a wrong
trellis path and \textbf{amplifies} errors instead of correcting them
(\emph{catastrophic FEC failure}) --- measured on a broken link as
\texttt{pre-FEC\ 24\%}, \texttt{post-FEC\ 42\%} (≈ random
=\textgreater{} genuine garbage). The processing gain of the preamble
correlation means a burst can \textbf{detect strongly yet decode to
garbage} (detection ≠ decodability).

Python: \texttt{marl\_phy.ber\_probe(n,\ scheme=...)} (or
\texttt{python3\ marl\_phy.py\ ber}) runs a warm AP with the known
payload, fires N copies, and reports min/median/max of both BERs --- a
quick link-quality meter at any distance.
...
manually with
\texttt{python3\ phy/tools/plot\_viz.py\ \textless{}dir\textgreater{}\ -\/-fs\ 1.6e6\ -\/-save\ out.png}.

\hypertarget{101-why-a-messy-constellation-can-still-decode-perfectly}{%
\subsubsection{\texorpdfstring{10.1 Why a \emph{messy} constellation can
still decode
perfectly}{10.1 Why a messy constellation can still decode perfectly}}\label{101-why-a-messy-constellation-can-still-decode-perfectly}}

A common and important surprise: the saved RX constellation can look
like a smeared ring --- the clusters visibly overlapping --- yet the
message decodes with zero errors. This is not a contradiction; it is the
whole point of the error-control stack. Four facts reconcile it.

\textbf{1. The plot is a single snapshot, not the data that decoded.}
\texttt{viz.hpp} writes each figure \textbf{once, from the \emph{first}
captured burst of the run} (\texttt{written\ once} / "only the FIRST
time called"). The message, by contrast, is assembled from \emph{many}
bursts (§8). So the picture you are staring at is one --- possibly poor
--- burst, while the message may have been built from later, cleaner
bursts you never see plotted. The snapshot over-represents the worst
moment.

\textbf{2. "Messy" is not "random."} A \emph{truly uniform} circle ---
phase spread evenly over \([0, 2\pi)\) --- genuinely cannot be decoded:
for \(M\)-PSK it gives an \(\approx (M-1)/M\) symbol-error rate, far
past any code's reach. The fact that decoding \emph{succeeds} is itself
proof the constellation is \textbf{not} uniform: the \(M\) clusters are
spread and overlapping but still statistically centred on the right
points, so \textbf{most} symbol decisions are still correct (often
80--90\%). The eye exaggerates the mess --- a scatter of thousands of
alpha-blended points plus a few outliers reads as a ring even when the
modes dominate.

\textbf{3. Three layers turn "mostly right" into "exactly right."} The
decision slicer's raw output is only the first stage:
...
FEC (§7) corrects a large fraction of the bit errors; CRC (§8) rejects
any chunk that still contains one; ARQ / repetition then retransmits
until a clean copy lands. The last layer is decisive: \textbf{you need
only ONE CRC-clean copy of each chunk.} A link that fails 80\% of bursts
still delivers a perfect message --- the passing 20\% are kept, the rest
dropped. The message is built from the \emph{lucky} bursts, not the
average one.

\textbf{4. Two different quantities.} The constellation/EVM measures the
\textbf{raw, uncoded} channel quality --- how hard the demod's job is.
The decoded message reports what survives \textbf{after} the coding
layers. They are \emph{allowed} to disagree, and the gap between them is
exactly the margin that FEC + CRC + retransmission buys:
...
Note the 8-PSK mapping here is \emph{natural binary}, \textbf{not}
Gray-coded (\texttt{modulator.cpp}
\texttt{create\_8psk\_constellation}), so none of this robustness comes
from a benign bit-assignment --- the coding layers do all of the work.
Switching to Gray coding (adjacent symbols differing by one bit) would
widen this margin further, a cheap future improvement.

This is also the precise mechanism behind the dense-QAM wall (§13):
there, even a \emph{single} clean copy never lands --- the raw symbol
errors exceed what FEC can carry and no burst passes CRC --- so
retransmission has nothing good to keep. 8-PSK sits right at the edge
where the raw symbols are borderline but the stack still closes the gap;
dense QAM/APSK is past it.
...
\textbf{Config file.} Every option below can be set in a file instead of
on the command line: \texttt{-\/-config\ phy.cfg} reads
\texttt{name\ =\ value} lines (\texttt{\#} comments; long option name
without \texttt{-\/-}), and anything on the command line
\textbf{overrides} the file --- so you keep one edited config and tweak
per-run. \texttt{phy.cfg} is a fully-defaulted template auto-generated
on build (\texttt{phy/tools/gen\_config\_template.py}) from
\texttt{sdr\_system\ -\/-help}, so it never drifts from the code.
...
\texttt{-\/-fec} & \texttt{false} & enable FEC (must match both ends) \\
\texttt{-\/-fec-type} & \texttt{conv} & \texttt{conv} / \texttt{ldpc} /
\texttt{turbo} (§7); must match both ends \\
\texttt{-\/-fec\_soft} & off & RX soft-decision (LLR) decoding (§7.4);
coherent schemes \\
\texttt{-\/-ldpc-k} & \texttt{256} & LDPC/turbo info-block size (bits);
must match both ends \\
\texttt{-\/-fec-iters} & \texttt{0} & max decoder iterations (0 =
default 50/6); decoder-only \\
\texttt{-\/-fec-scale} & \texttt{0} & min-sum/extrinsic scale (0 =
0.75); decoder-only \\
\texttt{-\/-ldpc-col-weight} & \texttt{3} & LDPC variable-node degree;
must match both ends \\
...
to \texttt{phy\_outputs/\textless{}scheme\textgreater{}/figure.png}. The
same set stands alone in the repo as \texttt{COMMANDS.md}, which also
carries adapted (untested) command templates for the \textbf{N210 / X310
/ X410} --- only the device args, subdev, antenna and gain range change;
the DSP flags are identical, and a shared 10 MHz reference
...
Hz. (The default CFO estimator is now the least-squares phase-slope of
§5.4-C, which \textasciitilde halves that jitter to ≈±450 Hz at 12 dB
SNR --- this \emph{widens the margin} for QPSK/8-PSK and the DQPSK range
test, but does \textbf{not} lift the wall below: the residual beat still
rotates a 16-point constellation faster than the DD PLL can track, so a
shared reference clock remains required for dense QAM.) On the cable,
the \textbf{TX carrier / LO leakage couples straight into the RX} (a
\textasciitilde40 dB spike near DC) and \textbf{beats at that CFO} --- a
slowly drifting near-DC tone. It (a) rotates the constellation, which
QPSK's wide decision regions survive but 16-QAM's don't (and the
decision-directed phase PLL can't lock 16 points), and (b) dominates the
...
\hypertarget{14-lora--css-chirp-spread-spectrum--decodable-receiver}{%
\subsection{14. LoRa / CSS (chirp spread spectrum) --- decodable
receiver}\label{14-lora--css-chirp-spread-spectrum--decodable-receiver}}

A parallel waveform to \texttt{sc}/\texttt{ofdm}, added in
\texttt{include/lora.hpp} with the radio path in
\texttt{phy/src/main.cpp} (\texttt{-\/-waveform\ lora}). Unlike the
constellation pipeline (m-sequence preamble + RRC + PLL), CSS has its
own chirp-based sync and demod, so it runs at the sample level via
\texttt{transmit\_samples} / \texttt{PHYSICAL\_LAYER::capture\_raw}
(like the tone path), bypassing the ACQ threads.

\hypertarget{modulation}{%
\subsubsection{Modulation}\label{modulation}}

Spreading factor SF -\textgreater{} N = 2\^{}SF chips per symbol, SF
bits/symbol. The base up-chirp is
\texttt{u(n)\ =\ exp(j·2π(n\^{}2/2N\ −\ n/2))}. Symbol
\texttt{s\ \ in\ \ {[}0,N)} is the base up-chirp cyclically shifted by
\texttt{s}: \texttt{x\_s(n)\ =\ u((n+s)\ mod\ N)}.

\hypertarget{demodulation-the-decodable-receiver}{%
\subsubsection{Demodulation (the decodable
receiver)}\label{demodulation-the-decodable-receiver}}

Dechirp (multiply by the conjugate base chirp = down-chirp) then FFT ---
the peak bin is the symbol. The FFT coherently integrates all N chips,
giving the LoRa \textbf{processing gain}: validated to decode at
\textbf{−10 dB SNR} (below the noise floor). Steps:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  \textbf{Preamble detection} --- slide and find \texttt{n\_preamble}
  up-chirps that dechirp to the \emph{same} bin with a \textbf{sharp}
  peak (peak/avg gated, so noise/constant junk can't false-lock).
\item
  \textbf{Fine timing} --- brute-force the sample shift τ in {[}0,N)
  that makes the SFD down-chirps decode cleanly; this pins the frame
  boundary and separates timing from CFO.
\item
  \textbf{CFO} --- the aligned preamble up-chirp peak gives the integer
  carrier-frequency offset (bins); de-rotated before data demod.
\item
  \textbf{Sync-word check} --- verify the 2 sync symbols (below); reject
  foreign frames.
\item
  \textbf{Data} --- dechirp+FFT argmax per symbol -\textgreater{}
  symbols -\textgreater{} bits -\textgreater{} CRC/FEC framing.
\end{enumerate}

\hypertarget{frame--sync-word}{%
\subsubsection{Frame + sync word}\label{frame--sync-word}}

\begin{verbatim}
[ preamble: 8 base up-chirps ][ sync word: 2 up-chirp symbols ][ SFD: 2 down-chirps ][ data symbols ]
\end{verbatim}

The \textbf{sync word} is a 1-byte network id sent as 2 symbols (each
nibble × 8, LoRa-style): \texttt{-\/-lora-sync-word} \textbf{default
0x12 (=18) private}, \texttt{0x34\ (=52)} public. The receiver rejects
frames whose sync symbols don't match --- network isolation. (Before
this, the scheme had \emph{no} sync word; the preamble was plain
up-chirps straight into the SFD.)

\hypertarget{includelorahpp-api}{%
\subsubsection{\texorpdfstring{\texttt{include/lora.hpp}
API}{include/lora.hpp API}}\label{includelorahpp-api}}

\begin{itemize}
\tightlist
\item
  \texttt{base\_chirp(SF,\ down)} --- base up/down chirp of N samples.
\item
  \texttt{mod\_symbol(s,\ SF,\ up,\ out)} /
  \texttt{demod\_symbol{[}\_q{]}(r,\ SF,\ ref{[},\ q{]})} --- one
  symbol; \texttt{q} is the peak/avg (sharpness) used to reject false
  locks.
\item
  \texttt{bits\_to\_symbols} / \texttt{symbols\_to\_bits} --- SF-bit
  packing.
\item
  \texttt{sync\_symbols(sync\_word,\ SF,\ s1,\ s2)} --- network id
  -\textgreater{} 2 symbols.
\item
  \texttt{modulate(bits,\ SF,\ n\_preamble=8,\ sync\_word=0x12)}
  -\textgreater{} complex samples (full frame).
\item
  \texttt{sync(r,\ SF,\ n\_preamble,\ cfo\_bins\&,\ sync\_word=0x12)}
  -\textgreater{} data-start sample index (−1 if no valid frame); fills
  the integer CFO.
\item
  \texttt{demodulate(rx,\ SF,\ nbits,\ n\_preamble=8,\ sync\_word=0x12)}
  -\textgreater{} bits.
\end{itemize}
...
\texttt{-\/-waveform\ lora}, \texttt{-\/-lora-sf} (7--12),
\texttt{-\/-lora-sync-word} (int; must match TX/RX). The raw jamming
sweep is separate: \texttt{-\/-message-type\ chirp} with
\texttt{-\/-chirp-bw/-\/-chirp-sf/-\/-chirp-down}.

\hypertarget{validation--status}{%
\subsubsection{Validation \& status}\label{validation--status}}

\texttt{tools/lora\_loopback\_test.cpp}
(\texttt{c++\ -std=c++17\ -O2\ -Iinclude\ tools/lora\_loopback\_test.cpp\ -o\ /tmp/lora\_test\ \&\&\ /tmp/lora\_test})
--- ALL PASS: clean, +137-sample timing offset, AWGN to −10 dB, integer
CFO, and sync-word rejection. The over-the-air
\texttt{-\/-waveform\ lora} link compiles and the Python API is wired,
but is \textbf{pending hardware validation} (needs the radios). One-way
only for now --- folding LoRa into
\texttt{sink\_arq}/\texttt{source\_arq} is the follow-up.
