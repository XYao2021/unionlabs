...
\textbf{Math.} A modulator maps \texttt{k\ =\ log2(M)} bits to one of
\texttt{M} complex points \texttt{s\ =\ I\ +\ jQ}:

\begin{itemize}
\tightlist
\item
  \textbf{M-PSK}: points on the unit circle,
  \texttt{s\ =\ exp(j·2π·m/M)}. All symbols equal-energy; demod by
  nearest angle. Robust but spectrally inefficient at high order (points
  crowd on the circle).
\item
  \textbf{M-QAM}: square/cross grid of amplitudes,
  \texttt{s\ =\ (2i−√M+1)\ +\ j(2q−√M+1)}. Best power efficiency for a
  given rate on an AWGN channel, but needs accurate amplitude
...
\item
  ...
  between PSK and QAM.
\item
  \emph{\emph{Differential (D})}*: information is in the \textbf{phase
  change} between consecutive symbols,
  \texttt{s\_n\ =\ s\_\{n-1\}·exp(jΔφ)}. No absolute phase reference
  needed -\textgreater{} tolerates carrier phase offset / no PLL, at a
  \textasciitilde3 dB SNR penalty. \texttt{PI4-QPSK} rotates the
  constellation by π/4 each symbol to avoid zero-crossings (lower
  envelope variation).
\end{itemize}
...
point \texttt{ŝ\ =\ argmin\_c\ \textbar{}y\ −\ c\textbar{}} nearest the
received symbol \texttt{y} (optimal for equiprobable symbols in AWGN).
An error happens when noise/distortion pushes \texttt{y} past the
halfway line to a neighbor, i.e. beyond half the \textbf{minimum
distance} \texttt{d\_min/2}. For a fixed average power \texttt{Es},
packing more points shrinks \texttt{d\_min}: QPSK has
\texttt{d\_min\ =\ √(2Es)} with the whole plane split into 4 quadrants,
while 16-QAM squeezes 16 points into the same power so \texttt{d\_min}
is \textasciitilde3× smaller and its decision cells \textasciitilde3×
tighter. The \textbf{error vector magnitude}
\texttt{EVM\ =\ rms(y\ −\ ŝ)/rms(c)} measures how far symbols land from
ideal; reliable decoding needs
\texttt{EVM\ \textless{}\textasciitilde{}\ d\_min/(2·rms)}, which is why
QPSK survived \textasciitilde37 \% EVM over the air but 16-QAM (needing
\textless\textasciitilde{} 12 \%) did not. \textbf{Gray coding} the
...
\textbf{Math.} A raised-cosine (RC) spectrum has zero inter-symbol
interference (ISI) at symbol instants (Nyquist criterion). Splitting it
as \textbf{RRC at TX and RRC at RX} makes the cascade a full RC
(matched-filter optimal in AWGN) \emph{and} maximizes SNR. Roll-off β
trades bandwidth \texttt{(1+β)·Rsym} against time-domain sidelobe decay.
...
\textbf{Math.} OFDM splits the band into \texttt{N} narrow orthogonal
subcarriers via the IDFT:
\texttt{x{[}n{]}\ =\ (1/N)\ Σ\_k\ X{[}k{]}·exp(j2πkn/N)}. A cyclic
prefix (copy of the tail prepended) turns the channel's \emph{linear}
convolution into \emph{circular} convolution, so after the FFT each
subcarrier sees a single complex gain \texttt{H{[}k{]}}:
\texttt{Y{[}k{]}\ =\ H{[}k{]}·X{[}k{]}\ +\ noise}. Equalization is then
\textbf{one complex divide per subcarrier}
(\texttt{X̂{[}k{]}\ =\ Y{[}k{]}/H{[}k{]}}) --- no FIR equalizer needed
even under heavy multipath, provided the delay spread \textless{} CP.
This is why OFDM is preferred for dense QAM on frequency-selective
channels. Cost: high peak-to-average power ratio (PAPR) and sensitivity
to carrier frequency offset (loss of subcarrier orthogonality
-\textgreater{} inter-carrier interference).

\textbf{Channel estimation \& pilots.} One known
\textbf{channel-estimation symbol} (known value on every active
subcarrier) gives \texttt{H{[}k{]}\ =\ Y\_chest{[}k{]}/ref{[}k{]}}.
...
The earlier version used the \emph{equalized} pilot
\texttt{Y{[}k{]}/H{[}k{]}}, which \textbf{blows up on a deep-fade
subcarrier} (tiny \textbar H{[}k{]}\textbar) --- one garbage pilot then
hijacked φ and smeared the whole symbol into a blob (EVM
\textasciitilde67\%, 0 CRC over the air; invisible in a mild simulation
channel). Weighting by channel power
\texttt{\textbar{}H{[}k{]}\textbar{}\^{}2} via \texttt{Y·conj(H)}
suppresses faded pilots instead of amplifying them -\textgreater{} four
clean clusters (EVM \textasciitilde37\%), message decodes. Disable with
env \texttt{OFDM\_NO\_CPE=1} to compare.
...
\texttt{H0}: noise only, \texttt{H1}: signal present --- on the received
power. The instantaneous power
\texttt{\textbar{}r{[}n{]}\textbar{}\^{}2} is smoothed by a one-pole IIR
(exponential moving average) to reduce variance:

\begin{verbatim}
filtered[n] = (1−α)·|r[n]|^2 + α·filtered[n−1]      (--alpha 0.95)
\end{verbatim}

Larger α = more averaging (time constant ≈ \texttt{1/(1−α)} ≈ 20
samples), which trades detection latency for a lower false-alarm rate
--- the smoothing collapses the variance of the noise power estimate so
a single threshold cleanly separates the two hypotheses. Declare a burst
when \texttt{filtered{[}n{]}\ \textgreater{}\ threshold}. (α = 0.02
barely smoothed -\textgreater{} the detector fired on every noise spike
and even chopped real bursts apart on the RRC envelope; α = 0.95 gives
one clean capture/burst.)

\begin{itemize}
\tightlist
\item
  \textbf{Adaptive threshold} (\texttt{-\/-det-adaptive\ true},
  default): \texttt{threshold\ =\ noise\_floor\ ×\ multiplier}, with
  \texttt{-\/-det-mult\ 5} (raise to 10--30 over the air so ambient RF
  doesn't trigger it; too high misses weak bursts).
\item
  \textbf{Continuous noise tracking} (\texttt{-\/-det-continuous\ true},
  default): the noise floor is re-estimated by an EMA during idle
  periods
  (\texttt{noise\_floor\ \textless{}-\ (1−β)·noise\_floor\ +\ β·inst}),
  so it follows drifting ambient noise. Alternative: one-shot startup
...
\tightlist
\item
  \textbf{A} --- unknown gain -\textgreater{} removed by the
  \textbf{AGC} (§9).
\item
  \textbf{Δf} --- \textbf{carrier frequency offset (CFO)}: the two
  radios' oscillators differ (each B210 TCXO is ±2 ppm, so up to
  \textasciitilde4 ppm ≈ 3.6 kHz at 915 MHz). A constant Δf produces a
  phase that \textbf{grows linearly with time},
  \texttt{θ{[}n{]}\ =\ 2π(Δf/fs)n} --- it \emph{spins} the
  constellation. Left uncorrected it turns the constellation into a
  ring.
\item
  \textbf{φ0} --- \textbf{static carrier phase offset}: the oscillators'
  phase difference at acquisition plus the channel's phase. A constant
  \emph{rotation} of the whole constellation.
\item
  \textbf{τ} --- \textbf{timing offset}: the ADC sampling grid isn't
...
\texttt{φ{[}n{]}\ =\ φ0\ +\ 2π(Δf/fs)n}}, which is why §5.5 uses a
\emph{second-order} loop (tracks a constant phase \textbf{and} its
constant rate) and why OFDM needs per-symbol CPE tracking (§3.2). The
recovery order is: \textbf{frame sync -\textgreater{} timing
...
A good preamble \texttt{p} has a \textbf{thumbtack autocorrelation}
\texttt{Σ\_n\ p{[}n{]}·conj(p{[}n+k{]})\ ≈\ E·δ{[}k{]}} (large at zero
lag, near-zero elsewhere) so the correlator (§5.2) gives one sharp,
unambiguous peak.
...
\item
  ...
  sequence. Periodic autocorrelation is two-valued: \texttt{L} at zero
  lag, \texttt{−1} otherwise. Real-valued.
\item
  \textbf{Zadoff-Chu} --- complex \textbf{CAZAC} sequence
  \texttt{p{[}n{]}\ =\ exp(−jπ\ u\ n(n+1)/L)}: \emph{constant amplitude}
  and \emph{zero cyclic autocorrelation}. The flat spectrum makes it
  ideal for training a complex channel estimate / equalizer, and its
  constant envelope correlates cleanly even under CFO. Preferred for
  equalizer training and dense QAM.
\end{itemize}
...
sequence in AWGN. At the true start \texttt{τ*}, all \texttt{L} terms
add coherently -\textgreater{}
\texttt{R(τ*)\ ≈\ Σ\textbar{}p{[}n{]}\textbar{}\^{}2}; at any wrong
offset the terms add with random phases -\textgreater{}
\texttt{R\ \textasciitilde{}\ √L} (much smaller). Detection:

\begin{itemize}
\tightlist
\item
  \textbf{Coarse pass}: find \texttt{τ} where \texttt{R(τ)} first
  crosses \texttt{-\/-sync\_threshold} (default 15), then
\item
...
\hypertarget{53-symbol-timing-recovery--gardner-ted}{%
\subsubsection{5.3 Symbol-timing recovery --- Gardner
TED}\label{53-symbol-timing-recovery--gardner-ted}}

The ACQ peak gets timing right to the nearest sample; the
\textbf{Gardner timing-error detector} (\texttt{GardnerTED}) then tracks
the residual \emph{fractional} delay continuously.
\texttt{-\/-timing\_loop\_bw\ 0.015} (loop bandwidth \texttt{Bn·T}),
\texttt{-\/-timing\_damping\ 0.707}, \texttt{-\/-sps\_sync\ 5}.

\textbf{Error detector.} With two samples per symbol --- one at the
symbol center \texttt{y{[}k{]}} and one at the half-symbol midpoint
\texttt{y{[}k−½{]}} --- the Gardner error is
...
\texttt{y{[}k−½{]}} (sitting on the transition between symbols)
correlates with the slope \texttt{y{[}k{]}−y{[}k−1{]}}; at perfect
timing the midpoint is a true zero-crossing and \texttt{e\ =\ 0}. It is
\textbf{decision-independent and carrier-phase-independent} (the
\texttt{Re\{·\}} with the difference term cancels a constant rotation),
so it works \emph{before} CFO/phase are resolved --- the reason Gardner
is the default choice for QAM.

\textbf{Loop.} The NCO advances \texttt{ω\ =\ 2/sps} per input sample
(two strobes/symbol --- using \texttt{1/sps} gives only one strobe and
the "midpoint" lands a full symbol away -\textgreater{} the loop tracks
garbage; this was a fixed bug). A \textbf{Farrow interpolator} produces
the fractional-delay sample at offset \texttt{μ}, and a
\textbf{proportional-integral (PI) loop filter} drives \texttt{μ}:
...
with \texttt{freq\_adj} clamped to ±10 \% of \texttt{ω}. The PI
coefficients come from the standard second-order design (same
\texttt{θn}/\texttt{ζ} formulas as §5.5). Only the symbol-center strobes
are emitted -\textgreater{} exactly one sample per symbol out.
...
A constant CFO spins the constellation at \texttt{2π·Δf/fs} rad/sample.
Two estimators are provided (\texttt{CFOEstimator}), both
\textbf{data-aided} off the preamble, then \texttt{FrequencyShifter}
removes it by multiplying sample \texttt{n} by
\texttt{exp(−j·2π·(Δf/fs)·n)} (a running phase accumulator, so blocks
join seamlessly).

\textbf{(A) Pilot-aided (phase-slope across preamble symbols).} For
preamble symbols \texttt{p{[}k{]}} received at \texttt{r{[}k·sps{]}},
the phase advance per symbol is measured with the known data stripped
off:
...
\texttt{\textbar{}Δf\textbar{}\ \textless{}\ fs/(2·sps)} (one symbol of
phase must not wrap).

\textbf{(B) Auto-correlation (Moose / Schmidl-Cox).} With a preamble
made of two identical halves of length \texttt{L}, the second half
equals the first times the CFO phase accrued over \texttt{L} samples:

\begin{verbatim}
P  = Σ_{n=0}^{L−1}  r[n+L] · conj(r[n])        (≈ |A|^2·L · e^{ j·2π·(Δf/fs)·L })
Δf = arg(P) · fs / ( 2π · L )
\end{verbatim}

Unambiguous range \texttt{\textbar{}Δf\textbar{}\ \textless{}\ fs/(2L)}
--- smaller \texttt{L} -\textgreater{} wider capture range but noisier.
This is the same estimator OFDM uses (§5.6), where \texttt{L\ =\ N/2}.

\textbf{Why residual CFO still matters.} \texttt{arg(·)} limits the
...
\texttt{φ{[}n{]}\ =\ φ0\ +\ (residual\ ramp)}. For \textbf{absolute}
constellations (BPSK/QPSK/QAM) all of it must be removed before hard
decisions; for \textbf{differential} schemes the static \texttt{φ0}
cancels automatically (demod uses phase \emph{differences}).
...
\item
  ...
  preamble: \texttt{φ̂\ =\ arg(\ Σ\_n\ r{[}n{]}·conj(p{[}n{]})\ )}.
\item
  \textbf{M-th power (blind, for M-PSK)} --- raising to the
  \texttt{M}-th power collapses all \texttt{M} data phases onto one,
  removing the modulation:
  \texttt{φ̂\ =\ (1/M)·arg(\ Σ\_n\ r{[}n{]}\^{}M\ )}.
\item
  \textbf{Decision-directed} ---
  \texttt{φ̂\ =\ arg(\ Σ\_n\ r{[}n{]}·conj(ŝ{[}n{]})\ )} against the
  nearest constellation points \texttt{ŝ} (used for QAM, after a coarse
  lock).
\end{itemize}
...
design for loop bandwidth \texttt{Bn·T}
(\texttt{-\/-phase\_loop\_bw\ 0.02}) and damping \texttt{ζ}
(\texttt{-\/-phase\_damping\ 0.707}):

\begin{verbatim}
θn = (Bn·T) / ( ζ + 1/(4ζ) )
α  = 4ζ·θn / ( 1 + 2ζ·θn + θn^2 )               (proportional gain)
β  = 4·θn^2 / ( 1 + 2ζ·θn + θn^2 )               (integral gain; β = 0 -> first-order loop)
\end{verbatim}

Smaller \texttt{Bn·T} = less jitter but slower pull-in;
\texttt{ζ\ =\ 0.707} is the critically-damped standard.
...
R(d) = Σ_{n=0}^{N/2−1} |r[d+n+N/2]|^2                (energy normaliser)
M(d) = |P(d)|^2 / R(d)^2                              (timing metric  in  [0,1])
\end{verbatim}

\texttt{M(d)} forms a \textbf{plateau} where the two halves align; the
frame start is the plateau's left edge plus a small guard into the CP
(any offset inside the CP is just a per-subcarrier phase the channel
estimate absorbs). The \textbf{fractional CFO} falls out of the same
correlation:
...
and the burst is derotated by \texttt{exp(−j·2π·ε·n/N)}. Residual CFO /
phase drift across the frame is then tracked per symbol by the
scattered-pilot CPE of §3.2. An energy gate (\texttt{R\ ≥\ 0.30·max\ R})
stops the metric from locking onto the low-power noise pad before the
burst. \emph{Alternatives:} Minn and Park metrics (sharper, no plateau
...
\textbf{Math.} A length-\texttt{T} linear FIR equalizer \texttt{w}
estimates each symbol from a window of received samples:
\texttt{ŝ{[}k{]}\ =\ w\^{}H·r{[}k{]}\ =\ Σ\_i\ conj(w\_i)·r{[}k−i{]}},
choosing \texttt{w} to invert the channel \texttt{h} (ideally
\texttt{w\ *\ h\ ≈\ δ}). Two ways to find \texttt{w}:

\begin{itemize}
\item
  \textbf{Least-squares / MMSE training} (used, exact for the complex
  Zadoff-Chu preamble). Stack the preamble windows into a matrix
  \texttt{R} and the known symbols into \texttt{s}; minimize
  \texttt{\textbar{}\textbar{}R·w\ −\ s\textbar{}\textbar{}\^{}2}:

\begin{verbatim}
w = (R^H R + λI)^(-1) R^H s          (λ = diagonal loading / MMSE regularization)
\end{verbatim}

  solved here by Gaussian elimination. \texttt{λI} = Tikhonov
  regularization: it bounds noise enhancement at channel nulls (the
  MMSE, not zero-forcing, solution).
\item
...
\begin{verbatim}
e[k] = s[k] − ŝ[k]
w   <- w + μ · e[k]* · r[k] / (||r[k]||^2 + ε)          (--eq_mu 0.3, 0 < μ < 2 for convergence)
\end{verbatim}
\end{itemize}

The LS solution places the main tap at the equalizer center, so a
\textbf{center-tap group delay} of \texttt{(T−1)/2} samples must be
...
\texttt{K−1\ =\ 6} input bits (state \texttt{σ\ \ in\ \ \{0..63\}});
each input bit \texttt{u} emits two coded bits as mod-2 (XOR) taps of
the register per the generator polynomials \texttt{G1\ =\ 171\_8},
\texttt{G2\ =\ 133\_8} (the standard NASA / 802.11a code):

\begin{verbatim}
c1 = G1 · [u, state]   (mod 2),   c2 = G2 · [u, state]   (mod 2)
\end{verbatim}

Each input bit thus influences \texttt{K\ =\ 7} output-bit pairs (the
...
received pair and each branch's expected \texttt{(c1,c2)}; the decoder
...
pattern up to roughly \texttt{floor((d\_free−1)/2)} ---
\texttt{d\_free\ =\ 10} for this code --- per window; validated
...
polynomial \texttt{M(x)} over GF(2); append 16 zeros
(\texttt{M(x)·x\^{}(16)}) and divide by the CRC-16-CCITT generator
\texttt{G(x)\ =\ x\^{}(16)+x\^{}(12)+x\^{}5+1} (mod-2 / XOR long
division). The 16-bit remainder is the CRC; the RX recomputes it and
flags any nonzero mismatch. Because an undetected error requires the
error polynomial \texttt{E(x)} to be an exact multiple of \texttt{G(x)},
this structure guarantees detection of: all single- and double-bit
errors, any odd number of errors (G has the factor \texttt{x+1}), and
all burst errors of length ≤ 16. Residual undetected probability ≈
\texttt{2\^{}(-16)} for random errors.
...
\hypertarget{12-verified-command-recipes-915-mhz-vert900-antennas}{%
\subsection{12. Verified command recipes (915 MHz, VERT900
antennas)}\label{12-verified-command-recipes-915-mhz-vert900-antennas}}

\textbf{QPSK OFDM + ARQ (reliable OTA demo).} Start the sink first.
...
\ExtensionTok{./sdr\_system} \AttributeTok{{-}{-}role}\NormalTok{ sink\_arq }\AttributeTok{{-}{-}rx{-}args}\NormalTok{ serial=30CD3F7 }\AttributeTok{{-}{-}tx{-}args}\NormalTok{ serial=30CD3F7 }\DataTypeTok{\textbackslash{}}
  \AttributeTok{{-}{-}rx{-}subdev}\NormalTok{ A:A }\AttributeTok{{-}{-}rx{-}ant}\NormalTok{ RX2 }\AttributeTok{{-}{-}rx{-}freq}\NormalTok{ 915e6 }\AttributeTok{{-}{-}rx{-}rate}\NormalTok{ 1.6e6 }\AttributeTok{{-}{-}rx{-}gain}\NormalTok{ 25 }\DataTypeTok{\textbackslash{}}
  ...
  \AttributeTok{{-}{-}ack{-}transport}\NormalTok{ tcp }\AttributeTok{{-}{-}ack{-}port}\NormalTok{ 5599 }\AttributeTok{{-}{-}det{-}mult}\NormalTok{ 3 }\AttributeTok{{-}{-}viz{-}dir}\NormalTok{ viz}
\CommentTok{\# Source / TX}
\ExtensionTok{./sdr\_system} \AttributeTok{{-}{-}role}\NormalTok{ source\_arq }\AttributeTok{{-}{-}tx{-}args}\NormalTok{ serial=30CD424 }\AttributeTok{{-}{-}rx{-}args}\NormalTok{ serial=30CD424 }\DataTypeTok{\textbackslash{}}
  \AttributeTok{{-}{-}tx{-}subdev}\NormalTok{ A:A }\AttributeTok{{-}{-}tx{-}ant}\NormalTok{ TX/RX }\AttributeTok{{-}{-}tx{-}freq}\NormalTok{ 915e6 }\AttributeTok{{-}{-}tx{-}rate}\NormalTok{ 1.6e6 }\AttributeTok{{-}{-}tx{-}gain}\NormalTok{ 82 }\DataTypeTok{\textbackslash{}}
  ...
  \AttributeTok{{-}{-}ack{-}transport}\NormalTok{ tcp }\AttributeTok{{-}{-}ack{-}host}\NormalTok{ 127.0.0.1 }\AttributeTok{{-}{-}ack{-}port}\NormalTok{ 5599 }\AttributeTok{{-}{-}timeout}\NormalTok{ 3000 }\AttributeTok{{-}{-}viz{-}dir}\NormalTok{ viz}
\end{Highlighting}
\end{Shaded}

\textbf{Single-carrier QPSK one-way} (swap
\texttt{-\/-waveform\ ofdm\ \ldots{}} for \texttt{-\/-waveform\ sc},
roles \texttt{rx}/\texttt{tx}, add \texttt{-\/-tx-reps\ 20}).

\textbf{16-QAM / 64-QAM}: same as above with \texttt{-\/-scheme\ 16QAM}
(or \texttt{64QAM}) on both ends --- but needs much higher SNR (EVM
\textless{} \textasciitilde12\% / 6\%). Use a cabled link (SMA + 30--40
dB attenuator) or very close antennas; tune \texttt{-\/-rx-gain} so the
RX \texttt{Peak=} stays under \textasciitilde0.8 (below 1.0 = ADC
clipping) while \texttt{RMS} is as high as possible. Best OTA point
found so far:
\texttt{-\/-rx-gain\ 21\ -\/-tx-gain\ 80\ -\/-ofdm-tx-peak\ 0.45}.
...
\tightlist
\item
  RX log \texttt{Peak\ \textgreater{}\ 1.0} -\textgreater{}
  \textbf{clipping}, lower \texttt{-\/-rx-gain} (fatal for QAM).
\item
  No detections (\texttt{bursts=0}) -\textgreater{} signal too weak,
  raise \texttt{-\/-tx-gain} or \texttt{-\/-rx-gain}, or lower
  \texttt{-\/-det-mult}.
\item
  Detections but CRC fails -\textgreater{} check the constellation EVM
  in the figure; if the order is too high for the link SNR, drop to QPSK
  or improve the link.
\item
