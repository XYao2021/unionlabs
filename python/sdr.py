...
OPTIONS = {
    ...
    "serve-forever": (False, None, "sink_arq: act as a persistent access point — keep the radio warm and re-accept a new source per session instead of exiting after one message (for fire-on-demand random access, e.g. the MARL bridge)."),
    "on-demand": (False, None, "source_arq: warm transmitter — keep the radio warm and send ONE packet each time a line is read on stdin, printing 'RESULT acked=0|1' per fire. No per-fire radio re-init (pairs with a --serve-forever AP)."),
...
class SDR:
    ...
    def __init__(self,
                 ...
                 serve_forever=_UNSET, # flag  sink_arq: act as a persistent access point — keep the radio...
                 on_demand=_UNSET, # flag  source_arq: warm transmitter — keep the radio warm and send...
